...
let getAllFeedBack = async (req, res) => {
  ...
  let feedbacks = await FeedBackModel.find({ store_id: id }).populate(
    "user_id"
  );
...
let CreateFeedBack = async (req, res) => {
  ...
  for (let i = 0; i < orders.length; i++) {
    ...
    if (
      loggedInUser.user_id == orders[i].user_id._id &&
      req.body.store_id == orders[i].store_id._id
    ) {
      flag = true;
      break;
    }
...
var deleteFeedBackByID = async (req, res) => {
  ...
  if (
    ...
    loggedInUser.user_id == feedbackToDelete[0].user_id._id
  ) {
